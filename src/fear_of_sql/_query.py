...
def _construct_result(result_type: type[T], row: Mapping[str, Any]) -> T:
    if dataclasses.is_dataclass(result_type) or hasattr(result_type, "model_fields"):
...
class Query(BaseQuery, Generic[T]):
    ...
    async def fetch_one(
        self, executor: asyncpg.Pool | asyncpg.Connection | psycopg.AsyncConnection
    ) -> T:
        row = await _async_executor(executor).fetch_one(self.sql, self.args)
        if row is None:
    ...
    async def fetch_all(
        ...
        rows = await _async_executor(executor).fetch_all(self.sql, self.args)
    ...
    def fetch_one_sync(self, conn: DBAPIConnection) -> T:
        ...
        if row is None:
            msg = "fetch_one: query returned no rows"
            raise RuntimeError(msg)
        return _construct_dbapi_result(self.result_type, _col_names(cursor), row)
...
class Execute(BaseQuery):
    ...
    async def execute(
        ...
        await _async_executor(executor).execute(self.sql, self.args)

    async def execute_rows(
        self, executor: asyncpg.Pool | asyncpg.Connection | psycopg.AsyncConnection
    ) -> int:
        return await _async_executor(executor).execute(self.sql, self.args)
